...
pis_tables_6 <- extract_tables("pis.pdf", output = "data.frame", pages=7)
...
pis_tables_6 <- data.frame(pis_tables_6[[1]]) %>%
  select(1:2) %>%
  filter(!startsWith(Name.Format, "Note"))

pis_tables_6$Name.Format[2] <- 
  paste(pis_tables_6$Name.Format[2],  pis_tables_6$Name.Format[3])

pis_tables_6 <- pis_tables_6 %>%
